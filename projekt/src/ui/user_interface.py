...
class UserInterface:
    ...
    def display_recommendations(self, routes, weather_data, date):
        print(f"\nRekomendowane trasy na dzień {date}:\n")
        for i, route in enumerate(routes, 1):
            duration = self.format_duration(route.estimate_time())
            key = f"{route.region}_{date}"
            weather = weather_data.get(key)
            category = route.categorize_route()
            ...
            if weather:
                print(f"   Komfort pogodowy: {weather.comfort_index}/100")
            else:
